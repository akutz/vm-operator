				...
				When("powering off the VM", func() {
					...
					When("power state should not be updated", func() {
						...
						When("vm has running task", func() {
							...
							When("there is also a task that no longer exists", func() {
								JustBeforeEach(func() {
									vmRef := vimtypes.ManagedObjectReference{
										Type:  string(vimtypes.ManagedObjectTypesVirtualMachine),
										Value: vm.Status.UniqueID,
									...
									reg.WithLock(
										...
										func() {
											vm := reg.Get(vmRef).(*simulator.VirtualMachine)
											vm.RecentTask = append(
												vm.RecentTask,
												vimtypes.ManagedObjectReference{
													Type:  string(vimtypes.ManagedObjectTypesTask),
													Value: "invalid",
												})
										})
								...
								It("should not return an expected error or change the power state", func() {
									err := createOrUpdateVM(ctx, vmProvider, vm)
									Expect(errors.Is(err, vsphere.ErrHasTask)).To(BeTrue(), fmt.Sprintf("%[1]T, %[1]s", err))
									Expect(vm.Status.PowerState).To(Equal(expectedPowerState))
								})
							})
						})
