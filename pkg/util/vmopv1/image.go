...
type ImageLinkedCloneInfo struct {
	ItemID             string
...
// GetImageLinkedCloneInfo returns the information about a VirtualMachineImage
// or ClusterVirtualMachineImage required to perform a linked clone operation
// using the image's disk(s).
func GetImageLinkedCloneInfo(
	ctx context.Context,
	k8sClient ctrlclient.Client,
	imgRef vmopv1.VirtualMachineImageRef,
	namespace string) (ImageLinkedCloneInfo, error) {

	img, err := GetImage(ctx, k8sClient, imgRef, namespace)
	if err != nil {
		return ImageLinkedCloneInfo{}, err
	}
